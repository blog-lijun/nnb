...
App({
  ...
  getOpenid(callback){
    ...
    if (self.globalData.openid){
      ...
      wx.login({
        success: res => {
          //console.log(res);
          if (res.code) {//登录成功后，获取open_id和session_key;
            wx.request({
  ...
  getTasksByStatus(callback,status){
    ...
    _this.getOpenid(function(err,openid){
      _this.getUserid(function(err,userid){
          if(userid){
            _this.getTasks(function(err,tasks){
                callback(null,tasks);
            },userid,status)
          }else{
  ...
  uploadImgs(callback,taskid,filepaths){
    ...
    _this.uploadImg(function(err,url){
      if(!err){
        success++
        url = url.push(url)
      }else{
      ...
      if(i==filepaths.length){
        ...
        data.fail = fail
        _this.uploadImgs(function(err,url){
          url = url.concat(url)
        },taskid,filepaths,data)
      }
    },taskid,filepaths[i])
